...
def save_metric_output(args, preds, frames, frame_id, motion_type, angle, frame_start, axis_dir='l'):
    ...
    output_dir = os.path.join(args.output, 'frame_{:0>4}'.format(frame_id+frame_start))
    ...
    norm = (np.array([[-1,0,0], [0,1,0], [0,0,-1]])@np.array([[-1,0,0],[0,-1,0],[0,0,1]])@normal.cpu().numpy().T).T
    np.save(f'{output_dir}/normal.npy', norm) 


def get_single_video_results(args, path, model, metadata, cls_name_map, frame_start, status):
    video_path = path
    is_video = True
    ...
    for i, im in enumerate(tqdm(reader)):
        ...
        if args.output is not None:
            ...
            # save 2D mask
            mask_num = str(i+frame_start+1).zfill(4)
    ...
    opt_preds, cluster, rsq, ref_idx, angles_tmp = optimize_planes(
    ...
    if args.save_obj:
        ...
        if ref_idx['trans'] != []:
            ...
            angles = angles_tmp['trans']
        else:
            frame_ids = ref_idx['rot']
            motion_type = 'rotation'
            angles = angles_tmp['rot']
        ...
        for frame_id in frame_ids:
            try:
                angle = angles[frame_id].cpu().numpy()[0].tolist()
            except:
                angle = []
            save_metric_output(args, opt_preds, frames, frame_id, motion_type, angle, frame_start)
            # pdb.set_trace()
    return frame_count
...
def main():
    ...
    for video in videos:
        ...
        # check if have different states videos
        path = f"../dataset/{video.replace('_b', '/b', 1)}/frames"
        if os.path.exists(f"{path}/video_open.mp4"):
            path_open = f"../dataset/{video.replace('_b', '/b', 1)}/frames/video_open.mp4"
            path_close = f"../dataset/{video.replace('_b', '/b', 1)}/frames/video_close.mp4"
            args.output = f"{output_dir}/{video.replace('_b', '/b', 1)}"
            os.makedirs(args.output, exist_ok=True)
            frame_count_open = get_single_video_results(args, path_open, model, metadata, cls_name_map, frame_start = 0, status=0)
            
            # pdb.set_trace()
            frame_count_close = get_single_video_results(args, path_close, model, metadata, cls_name_map, frame_start = frame_count_open, status=1)
            ...
            args.output = f"{output_dir}/{video.replace('_b', '/b', 1)}"
            os.makedirs(args.output, exist_ok=True)
            get_single_video_results(args, path, model, metadata, cls_name_map, frame_start = 0, status=0)

        end_time = time.time()
        time_one_video = {video : end_time - start_time}
        time_video.append(time_one_video)
    with open(f'{output_dir}/time.json', 'w') as fp:
        json.dump(time_video, fp)
